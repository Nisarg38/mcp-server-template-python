...
async def test_calculator_tools(mcp_server):
    ...
    assert (
        "5" in response_data["expression"]
        and "7" in response_data["expression"]
        and "12" in response_data["expression"]
    )
...
async def test_language_resources(mcp_server):
    ...
    # Test languages list
    languages_response = await mcp_server.read_resource(
        "http://localhost/programming/languages"
    )
    ...
    python_response = await mcp_server.read_resource(
        "http://localhost/programming/languages/python"
    )
    ...
    invalid_response = await mcp_server.read_resource(
        "http://localhost/programming/languages/brainfuck"
    )
...
async def test_prompts(mcp_server):
    ...
    math_prompt_response = await mcp_server.get_prompt(
        "math_problem", {"problem": "Solve for x: 2x + 5 = 13"}
    )
    ...
    lang_prompt_response = await mcp_server.get_prompt(
        "language_comparison", {"languages": ["Python", "JavaScript", "Go"]}
    )
    ...
    assert (
        "Please compare the following programming languages: Python, JavaScript, Go"
        in lang_text
    )
